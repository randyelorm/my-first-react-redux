...
export const addGrade = (a_grade) => {
    ...
    return (dispatch, state, { getFirestore }) => {
        getFirestore().collection("Self-development").add(a_grade) // it's more convenient to let Cloud Firestore auto-generate an ID for you. You can do this by calling add():  
            .then(
...
export const getAllgrades = () => {
    ...
    return (dispatch, state, { getFirestore }) => {
        ...
        getFirestore().collection("Self-development").onSnapshot(
            (snapshot) => {
                let exams_grades = []
                snapshot.forEach(
                    (items_in_database) => {
...
export const deleteGrade = (grade_id) => {
    ...
    return (state, dispatch, { getFirestore }) => {
        getFirestore().collection("Self-development").doc(grade_id).delete().then(() => {
...
export const editGrade = (updated_form) => {
    ...
    return (dispatch, state, { getFirestore }) => {
        ...
        getFirestore().collection("Self-development").doc(updated_form.id).update(updated_form)
            .then(() => {
